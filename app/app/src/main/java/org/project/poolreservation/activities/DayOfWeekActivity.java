...
public class DayOfWeekActivity extends AppCompatActivity {
    ...
    TextView title;
    RelativeLayout filterLayout,noSansLayout;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        daYArray= getIntent().getStringArrayListExtra("dayArray");
        ...
        noSansLayout=findViewById(R.id.noSans);
    ...
    private void setDateFake() {
       ...
       /* times.add(new TimeOfDay("10-12", 0, 5, 20));
        ...
        System.out.println("------------"+size+"---"+daYArray+"___"+dateArray);
        for(int i=0;i<size;i++)
            dayOfWeekList.add(new DayOfWeek(daYArray.get(i), dateArray.get(i).replace("\\/", "-")));
...
/*
    ...
    public void onBackPressed() {
        if(user_role.equals("Owner")) {
            Toast.makeText(DayOfWeekActivity.this, "back pressed", Toast.LENGTH_SHORT).show();
            startActivity(new Intent(DayOfWeekActivity.this,Public_register_Activity.class));
            finish();
        }
...
//request a json object response
        JsonObjectRequest jsonRequest = new JsonObjectRequest(com.android.volley.Request.Method.POST, url, jsonBody, new com.android.volley.Response.Listener<JSONObject>() {
            ...
            public void onResponse(JSONObject response) {
                ...
                //now handle the response
                Toast.makeText(DayOfWeekActivity.this, response.toString(), Toast.LENGTH_SHORT).show();
                System.out.println("--------------------day-of-week-2-------------------");
                System.out.println(response.toString());
                ArrayList<String> sectionArray=new ArrayList<>();
                if( response.toString().contains("200")){
                    String capacity="";
                    int capacity_index=response.toString().lastIndexOf("pool_capacity")+16;
                    while (true){
                        capacity+=response.toString().charAt(capacity_index);
                        capacity_index++;
                        if(response.toString().charAt(capacity_index)=='"')
                            break;
                    }
                    ...
                    timeOfDayList.clear();
                    for(int i=0;i<genderArr.size();i++) {
                        timeOfDayList.add(new TimeOfDay(sectionIdArr.get(i), priceArr.get(i), startHourArr.get(i) + " - " + endHourArr.get(i),
                                genderArr.get(i), Integer.valueOf(capacity)-Integer.valueOf(reservedNumArr.get(i))));
                    }
                   ...
                   /* timeOfDayList.add(new TimeOfDay("1", "0", "5", "male", 2));
                    ...
                    if (timeOfDayList != null && timeOfDayList.size() > 0) {
                        ...
                        timeOfDayRecy.setAdapter(new TimeOfDayAdaptor(timeOfDayList, DayOfWeekActivity.this,sectionIdArr,token));
